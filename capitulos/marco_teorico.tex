...
Desde una perspectiva técnica, el Business Intelligence (BI) transita de ser una solución de nicho, confinada a informes estáticos, a convertirse en un ecosistema abierto de Big Data que actúa como el sistema nervioso central de las organizaciones modernas \parencite{tagarro2019desarrollo}. Históricamente, el BI depende de arquitecturas propietarias cerradas que fuerzan ciclos rígidos de procesamiento por lotes. En la actualidad, la competitividad se define por la agilidad en el procesamiento de volúmenes masivos de información, donde la capacidad de transformar datos brutos en decisiones ejecutivas en cuestión de minutos marca la diferencia entre el liderazgo sectorial y la obsolescencia operativa.
...
El modelo resultante suele adoptar la forma de un \textbf{Esquema de Estrella}, donde una tabla de hechos central está rodeada por tablas de dimensiones desnormalizadas \parencite{kimball2013data}. El \textbf{Esquema de Copo de Nieve} es una variación donde las dimensiones se normalizan parcialmente, aunque suele evitarse en entornos analíticos puros por la complejidad que añade a las consultas SQL.
...
La transición del Data Warehouse tradicional al Data Lake no es solo una actualización de software, sino un cambio de paradigma hacia la agilidad empresarial extrema \parencite{tagarro2019desarrollo}. Mientras el DW exige que el dato se adapte a un molde predefinido antes de guardarse, el Data Lake permite que la empresa trabaje con sus datos en formato nativo, postergando la estructura hasta el momento preciso del consumo.
...
Un \textbf{Pipeline de Datos} constituye la infraestructura esencial para mover información desde un sistema de origen hacia un destino, aplicando transformaciones críticas en el proceso \parencite{harenslak2021data}. Dada la creciente complejidad de estos flujos, la orquestación se vuelve indispensable.

Apache Airflow, siguiendo la filosofía ``Workflow as Code'' \parencite{harenslak2021data}, aborda esta necesidad al permitir la definición de flujos de trabajo mediante código Python, lo que facilita el uso de control de versiones, pruebas unitarias y la colaboración entre ingenieros. La representación lógica de este pipeline se materializa en un \textbf{Grafo Acíclico Dirigido (DAG)}, cuyas propiedades fundamentales son:
...
DuckDB representa una innovación en el ámbito de las bases de datos analíticas embebidas \parencite{needham2024duckdb}. Se destaca su capacidad para realizar procesamiento OLAP directamente en el entorno de ejecución del usuario (\textit{in-process}), eliminando la latencia de red de los sistemas cliente-servidor tradicionales.

A diferencia de los motores de bases de datos tradicionales que procesan una fila a la vez, DuckDB utiliza \textbf{ejecución vectorizada}, procesando grandes bloques de datos simultáneamente \parencite{needham2024duckdb}. Esta técnica optimiza el uso de la caché de la CPU y permite un rendimiento superior en consultas complejas sobre grandes volúmenes de datos.
...
El gobierno de datos (\textit{Data Governance}) se define como el ejercicio de la toma de decisiones y la autoridad sobre los activos de datos \parencite{ladley2019data}. Sus componentes clave incluyen:
...
Además, según \textcite{ladley2019data}, la gobernanza se apoya en roles fundamentales:
...
\newpage
\begin{itemize}
    \item \textbf{Ventas y CRM:} Control de ventas y relación con clientes.
